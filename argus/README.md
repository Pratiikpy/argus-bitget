...
- Every decision is hash-chained **before** its outcome exists, and **256 claims across the
...
been *no* **every time it has been asked** — and the interesting output is not the trade, it is the
record that says why.

**The obvious rebuttal is that it never got the chance, and the record refutes it.** 349 of those
673 decisions were taken in a tradeable session with the anchor market open, and **344 of them
...
<!-- The plan and the third-party licence ledger are working documents and are deliberately
     not published: the plan carries internal scheduling, and the ledger sits under
     `research/`, which is excluded because it holds hundreds of cloned upstream repos that
     are not ours to redistribute. Both were linked here and both 404'd for any reader of
     the public repo. The licence position they document is stated inline below instead. -->
...
```
6,700 tests collected   ruff clean   mypy --strict clean on 352 source files
152/152 modules importable           18/18 sub-themes resolve to a symbol and a test file
673 decisions in the paper ledger (as of 2026-09-22)  chain verifies, head anchor agrees, no truncation
...
```bash
python -m argus.paper.runner --report    # Sharpe, max drawdown, win rate — or the reason each is undefined
...
python -m argus.eval.riskproof           # sweep the risk layer's whole input domain (~5 min, silent until done)
...
python -m argus.research.cointegration     # which stock-perpetual pairs are actually cointegrated (66 tests, corrected; ~2 min)
python -m argus.risk.effectiveness        # how much risk the anchor hedge actually removes, per session phase (~5 min; 36 paginated venue fetches)
...
python -m argus.market.markout --symbol MSTRUSDT --seconds 150   # how badly a resting order is picked off here
...
python -m argus.eval.refusal             # did standing aside cost money, or save it
...
| **The maker-side assumption, finally measured** | `market/markout.py` | `cost/model.py` refuses to credit maker treatment at all, because our own replay turned a +90% simulation into -0.45% once resting orders were modelled honestly. Right call, and a blanket assumption: nothing had measured *how much* adverse selection costs here. The venue's public fills feed (`catalog.ts:118` — the second endpoint we had never called) carries the aggressor side and a millisecond stamp, so the book is polled at 1Hz and joined to the prints. **Live on MSTRUSDT, 150s, weekend: three consecutive runs on 2026-09-20 put the passive side at a median -0.8 to -2.9bps, then -0.3 to -2.9bps, then **+0.3 to +1.6bps** — the sign itself is not stable at this sample size.** That instability is the finding, not a caveat on it: 150 seconds of one instrument with the anchor shut carries too little informed flow to determine even the direction of adverse selection. Run `python -m argus.market.markout --symbol MSTRUSDT --seconds 150` twice and you will see it move. The sign convention is the whole module — a flipped sign is symmetric, plausible, and says resting orders get picked *up* — so five tests assert it from both directions. The module's printed verdict follows whichever sample it drew, and across those three runs it said *refuse to change the cost model*, then *this deserves a number rather than a rule*, then *no measurable adverse selection*. **`cost/model.py` is unchanged by all three** — a conclusion that reverses between consecutive 150-second windows is not evidence about the venue, and the same run on NVDAUSDT drew six joined prints and correctly concluded nothing at all. The maker assumption moves when a regular-hours sample clears the print minimum at the longer horizons and holds its sign across runs. Sampling now runs on every scheduled cycle, and the schedule sits in US regular hours where the informed flow is. Sampling now runs on every scheduled cycle, and the schedule sits in US regular hours where the informed flow is (`data/markout.json`, `research/architecture/34-markout-and-adverse-selection.md`). |
...
| **What a trade really costs, taken from the live book** | `market/depth.py` | The paper ledger charged the **quoted touch** on entry and a flat **0.6bps** on every exit. The quote is the price of an infinitesimal trade; nothing in the system had ever asked what it costs to take a real size. `GET /api/v3/market/orderbook` is public, keyless and up to 200 levels — it sits in Bitget's own SDK catalogue (`agent-sdk/src/generated/catalog.ts:115`) and `market/bitget.py` had never called it. Now the book is walked level by level for the notional actually being traded. **Measured live on 2026-09-20, slippage against mid: at $25,000 QQQUSDT costs 2.27bps and SQQQUSDT 12.84bps, against the half-spread an infinitesimal order would pay of 0.07 and 1.31 — the quote understates the real cost 32x on one and 10x on the other, and a factor that varies across instruments is worse than a constant one because it reorders which names look cheap.** (Both figures are a single snapshot of a live book and move on every run; `python -m argus.market.depth` reprints them. The comparison is against the *half*-spread deliberately: the table's `quoted` column carries the full touch-to-touch spread, which is twice the mid-relative number the size columns report.) A size the visible book cannot absorb returns *no* measurement rather than the cheap half of a trade that could not be done, and the ledger then charges its documented stand-in. Both legs of every paper fill are now priced this way, and the scheduled cycle refreshes the table before each run (`data/depth.json`, `research/architecture/32-order-book-depth-teardown.md`). |
...
| **The risk layer proved over its whole domain, not sampled by the market** | `eval/riskproof.py` | Track 2 scores risk-control effectiveness, and ours had fired zero times in the 447 live records the sweep was run against (2026-09-20; the ledger has since reached 495, all of them still abstentions) — every decision so far has been an abstention, which proposes no exposure to narrow. A risk layer is a function, so it is proved instead of awaited: **2,177,280 states** — every combination of verdict, side, size straddling each limit, confidence straddling the floor, hedge availability, NAV staleness and session phase — each checked against the six properties that make it a risk layer. It never increased exposure, never reversed a side, never turned an abstention into a trade, never changed anything under ALLOW, never originated a leg of its own, and never raised net directional exposure — gross notional is deliberately *not* asserted monotone, because a hedge leg raises it while lowering net risk. It narrowed the proposal in **1,360,008 states (62%)**. **All fifteen configured rules bind somewhere, so none is shadowed**, and the precedence between overlapping rules is derived from the sweep rather than read off the source (`data/risk_proof.json`). The harness is itself tested against deliberately broken policies — one that doubles the position, one that flips the side, one that swallows every state — and must catch each, because a prover that cannot fail proves nothing. |
...
| **Beta that knows which session it is** | `desk/portfolio.py` | Open-session beta exceeds shut-session beta on 10 of 11 stock perpetuals; AAPL reads 0.68 open against 0.105 shut. A single blended beta is dominated by the ~82% of bars where the anchor market is shut, so it understates exposure in the only session that prices the underlying. |
...
- **Zero positions have settled.** All 673 ledger entries are refusals, and 481 have settled as
...
- **The `side` field carries no information, and that was measured rather than assumed.** Over the
  ...
  (`eval/shadow.py`). **506 decisions carry a lean** in the shadow record, UP and DOWN at differing
...
- **The other thirteen are answered anyway, from the sources those Skills name.** The same calls
  fail through a second, independent MCP client, and every upstream answers when called directly
...
- **20 of 43 capabilities are OWNED; one more reaches twelve of thirteen and stops there for a
...
- **Four modules existed, were tested, and ran on nothing — and one parser was inventing prices.**
  ...
  and the universe re-verifies every cycle — live, all twelve stock perpetuals still attenuate 3.80x-8.54x.
...
| `data/session_beta.json` | Beta per stock perpetual in the open session, the shut session, and blended |
...
| `research/architecture/*.md` | 56 code-level teardowns of the systems this was built against, each citing `file:line` |
