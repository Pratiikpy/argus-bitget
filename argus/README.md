...
- Every decision is hash-chained **before** its outcome exists, and **216 claims across the
...
been *no* **585 times out of 585** — and the interesting output is not the trade, it is the
record that says why.

**The obvious rebuttal is that it never got the chance, and the record refutes it.** 265 of those
585 decisions were taken in a tradeable session with the anchor market open, and **260 of them
...
585 decisions in the paper ledger (as of 2026-09-22)  chain verifies, head anchor agrees, no truncation
...
- **Zero positions have settled.** All 585 ledger entries are refusals, and 481 have settled as
...
- **The `side` field carries no information, and that was measured rather than assumed.** Over the
  ...
  (`eval/shadow.py`). **418 decisions carry a lean** so far — already UP and DOWN at differing
