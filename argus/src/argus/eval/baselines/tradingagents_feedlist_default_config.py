...
#
# `DEFAULT_CONFIG["data_vendors"]` here is TradingAgents' own real default vendor chain per
# category (yfinance for core/technical/fundamental/news, fred for macro, polymarket for
# prediction markets) — the comparison reads this dict directly rather than re-typing the
# category → vendor mapping, so a future upstream change in the default routing is caught by the
# pinned-hash drift test rather than silently going stale here.
#
# Apache License
