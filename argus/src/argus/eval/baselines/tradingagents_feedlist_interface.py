...
#
# This is TradingAgents' real data-vendor routing layer: `route_to_vendor()` (defined further down
# this file), the `VENDOR_METHODS`/`TOOLS_CATEGORIES`/`OPTIONAL_CATEGORIES` registries, and the
# `VendorError` taxonomy it dispatches on. `argus.eval.feedlist_comparison` runs this real function
# — not a paraphrase of it — against synthetic vendor implementations that raise
# `VendorRateLimitError`/`VendorNotConfiguredError`/`NoMarketDataError`/a bare `Exception` on
...
# `from .y_finance import ...`, `from .yfinance_news import ...`) are satisfied by a sys.modules
# shim in `tradingagents_feedlist_loader.py` — the sibling vendor-implementation modules
# (alpha_vantage, fred, polymarket, y_finance, yfinance_news) are stubbed with placeholder
# functions under the exact names this file imports, since `route_to_vendor()`'s own control flow
# (the part under comparison) never calls through to a real network vendor implementation — the
# comparison supplies its own controlled stand-ins directly into `VENDOR_METHODS` at runtime,
# exactly as a caller of the real TradingAgents interface would configure a vendor, so this file
# needed zero import-path or logic edits to run unmodified. `errors.py`, `config.py` and
# `default_config.py` (this file's other real dependencies) are vendored alongside it, unedited.
#
# Apache License
