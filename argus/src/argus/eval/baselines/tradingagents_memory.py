...
#
# This is TradingAgents' real episodic-memory log: `TradingMemoryLog` (defined further down this
# file) is the class argus.eval.recall_comparison runs, unmodified, as the `baseline_reproduced` /
# `same_input_comparison` evidence for "Episodic memory across decisions" in eval/standing.py,
...
# `tradingagents_memory_loader.py`, so this file needed zero import-path edits to run unmodified.
# `TradingMemoryLog.__init__`'s `store_decision`/`update_with_outcome` write path (which needs a
# real filesystem path) is exercised directly by the comparison — no reflection LLM call is
# needed or made anywhere in this comparison: `reflect_on_final_decision`
# (`tradingagents/graph/reflection.py`, NOT vendored — it is a thin LLM prompt-and-invoke wrapper
# with no logic of its own to compare) supplies the prose string that `update_with_outcome` stores
# verbatim; this comparison supplies its own fixed prose strings directly instead, which exercises
# exactly the same storage/retrieval code path this file owns.
#
# Apache License
