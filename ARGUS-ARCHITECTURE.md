...
**Live:** 196 claims across the twelve rTokens (`data/register.jsonl`), chain intact. The opening anchored head `bc36478291a06bc3` is claim 36 of 156 — the head at the moment of that anchoring, so that proof covers the first 36 claims and not the 120 registered since; each scheduled cycle appends and re-anchors, and 38 of the 56 proofs carry a Bitcoin block-header attestation (`python -m argus.register.anchorcheck`). Anchored to
